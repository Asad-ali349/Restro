...
if ($_POST['table_id']&&$_POST['food_id']&&$_POST['quantity']&&$_POST['hotel_id']&&$_POST['']) {
	...
	if (mysqli_num_rows($result_sql)>0) {
		while ($data=mysqli_fetch_array($result_sql)) {
			if ($data['table_id']==$tableId&&$data['food_id']==$foodId) {
				...
				//echo $hotelId; 
				$update_quantity="UPDATE cart SET quantity='$qua',total_price='$totalprice',discount_price='$disPrice' WHERE table_id='$tableId' AND food_id='$foodId' AND hotel_id='$hotelId'";
		...
		echo $hotelId; 
		$insert="INSERT INTO cart( table_id, food_id, quantity,total_price,discount_price,hotel_id) VALUES ('$tableId','$foodId','$quantity','$totalprice','$disPrice','$hotelId')";
